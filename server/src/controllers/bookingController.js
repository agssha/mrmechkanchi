...
class BookingController {
    ...
    /**
     * Mechanic adjusts applied coupon discount
     */
    async applyCouponDiscount(req, res, next) {
        try {
            const { bookingId, discountAmount } = req.body;
            const mechanicPhone = req.user.phone;
            const booking = await bookingService.applyCouponDiscount(bookingId, discountAmount, mechanicPhone);
            return ApiResponse.success(res, "Coupon discount applied successfully", { booking }, 200);
        } catch (error) {
            next(error);
        }
    }
